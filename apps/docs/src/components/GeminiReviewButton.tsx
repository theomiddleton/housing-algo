...
function buildGeminiUrl(): string {
  ...
  return `https://gemini.google.com/?prompt_text=${encodedPrompt}`;
}

interface GeminiReviewButtonProps {
...
export function GeminiReviewButton({
  variant = 'outline',
  size = 'default',
  showDescription = true,
  className = ''
}: GeminiReviewButtonProps) {
  const handleClick = () => {
    window.open(buildGeminiUrl(), '_blank', 'noopener,noreferrer');
  };
